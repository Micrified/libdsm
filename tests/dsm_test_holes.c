...
int main (void) {
	...
	// Register 0 -> 2 as "a".
	assert((a = dsm_dig_hole(0, 2, &g_shm_holes)) != -1);

	// Register 2 -> 4 as "b".
	assert((b = dsm_dig_hole(2, 2, &g_shm_holes)) != -1);

	// Register 6 -> 7 as "c".
	assert((c = dsm_dig_hole(6, 1, &g_shm_holes)) != -1);
